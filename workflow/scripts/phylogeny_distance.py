...
def get_options():
    ...
    parser.add_argument("phylogeny", help="Tree file")

    parser.add_argument(
        "--format", default="newick", help="Format of tree file [Default: newick]"
    )
    parser.add_argument(
        "--midpoint",
        action="store_true",
        default=False,
        help="Midpoint root the tree before calculating distances.",
    )
...
if __name__ == "__main__":
    ...
    tree = dendropy.Tree.get(
        path=options.phylogeny, schema=options.format, preserve_underscores=True
    )
